...
const ReviewItem: React.FC<ReviewItemProps> = ({
  ...
  return (
    <div className="border-b border-gray-100 py-6">
        ...
        {isOwner && (
          <div className="flex gap-2">
            <button
              ...
              className="!text-[14px] text-gray-500 hover:text-[#00cfcf] transition-colors"
            >
              수정
            </button>
            <button
              onClick={() => onDelete(review)}
              className="!text-[14px] text-gray-500 hover:text-red-500 transition-colors"
            >
              삭제
            </button>
          </div>
      ...
      {/* 리뷰 내용 */}
      <p className="text-gray-700 text-md leading-relaxed">{review.content}</p>
